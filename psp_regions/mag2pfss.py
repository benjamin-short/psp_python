...
import sunpy.map, glob, h5py
from sunkit_magex import pfss
from sunpy.coordinates import sun
import matplotlib.pyplot as plt
from scipy.interpolate import interp1d

DefaultPath = os.environ.get("SUNPY_DATA_DIR")
...
def gong2pfss(dt,rss=2.5,nr=60,ret_magnetogram=False,data_dir=DefaultPath) :
	...
	filepath = sorted(glob.glob(f'{data_dir}/gong/mrzqs{YYYYMMDD[2:]}*.fits'))[0]
	...
	peri_input = pfss.Input(br_gong, nr, rss,dtime=dt)
	peri_output = pfss.pfss(peri_input)

	return peri_output

def adapt2pfss(dt,rss=2.5,nr=60,adapt_source="GONG",realization="mean",
				 ret_magnetogram=False,data_dir=DefaultPath):
	#Input : dt (datetime object), rss (source surface radius in Rs)    
	YYYYMMDD = f'{dt.year}{dt.month:02d}{dt.day:02d}'
	A = {"GONG": 3, "HMI" : 4}.get(adapt_source)
	filepath=sorted(glob.glob(f"{data_dir}/adapt/adapt40{A}*{YYYYMMDD}*.fts"))[0]
	stdout.write(f"Read in {filepath}\r")
	...
	peri_input = pfss.Input(br_adapt, nr, rss,dtime=dt)
	peri_output = pfss.pfss(peri_input)

	return peri_output

def derosa2pfss(dt,rss=2.5,nr=60,
	...
	filepath = f'{data_dir}/derosa/Bfield_{YYYYMMDD}{dr_time}'
	...
	peri_input = pfss.Input(br_dr_interp, nr, rss,dtime=dt)
	peri_output = pfss.pfss(peri_input)	
	return peri_output

def hmi2pfss(dt,rss=2.5,nr=60,ret_magnetogram=False,data_dir=DefaultPath) :
	#Input : dt (datetime object), rss (source surface radius in Rs)
	YYYYMMDD = f'{dt.year}{dt.month:02d}{dt.day:02d}'
	filepath = glob.glob(f'{data_dir}/hmi/hmi.mrdailysynframe_small_720s.{YYYYMMDD}*.fits')[0]
	stdout.write(f"Read in {filepath}\r")
	...
	peri_input = pfss.Input(br_hmi, nr, rss,dtime=dt)
	peri_output = pfss.pfss(peri_input)
	return peri_output
