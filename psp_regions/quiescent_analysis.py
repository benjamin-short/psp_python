    ...
    for i in enc:
        ...
        if atype in ptypes:
            ...
            if atype in ['panis','ptpar','ptper']:
                ...
                Txx = tens_data_arr[:,0]
                Tyy = tens_data_arr[:,1] # SUSPECT TENSOR TERM??
                Tzz = tens_data_arr[:,2]
                Txy = tens_data_arr[:,3]
                Txz = tens_data_arr[:,4]
                Tyz = tens_data_arr[:,5]
...
def brazil(enc='no 1',enc_radius=45,plot='all'):
    ...
    if enc == 'no 1':
        enc = list(range(2,23)) #shortcut to exlude encounter 1, we use SPC and it gets weird.
        title_mod = 'for All Encounters, no 1'
...
def quiet_ham(enc=11,enc_radius=55,save=False):
    ...
    csv_files = glob.glob(ham_path)
    
    # Read all CSV files and concatenate them into a single DataFrame
    combined_df = pd.concat([pd.read_csv(file) for file in csv_files], ignore_index=True)
    
    ham_time = combined_df['time'].to_numpy()
    ham_data = combined_df['hamogram_2m'].to_numpy()
    
    # Convert the 'time' column to datetime
    combined_df['datetime'] = pd.to_datetime(combined_df['datetime'],format='mixed')
    
    ham_datetime = combined_df['datetime'].to_numpy()
    # breakpoint()
    for i in enc:
    
        t0, tf = utils.encounter_dates(i,enc_radius)
        t0_flt = pys.time_float(t0)
        tf_flt = pys.time_float(tf)    
    
        where = np.where((ham_time>t0_flt)&(ham_time<tf_flt))
        
        ham_time_arr = ham_time[where]
        ham_data_arr = ham_data[where]
        ham_datetime_arr = ham_datetime[where]
        
        q_time, non_q_time, q_index_arr = utils.find_quiescent_points(i,ham_time_arr)
        
        q_ham_time = q_time
        q_ham_datetime = ham_datetime_arr[q_index_arr]
        q_ham = ham_data_arr[q_index_arr]
        
        t0_min = np.min(ham_time_arr)
        tf_max = np.max(ham_time_arr)
        
        t0_mag = pys.time_string(t0_min)
        tf_mag = pys.time_string(tf_max)
        ...
        mag_datetime_arr = pd.to_datetime(mag_time, unit='s').to_numpy()
        
        q_mag_time, non_q_mag_time, q_mag_index_arr = utils.find_quiescent_points(i,mag_time)
        
        q_mag_Br = Br[q_mag_index_arr]
        q_mag_datetime = mag_datetime_arr[q_mag_index_arr]
        ...
        ax1.set_title("Quiescent Ham, Encounter "+str(i),fontsize=38)
        
        ax1.set_ylabel('Magnetic Field (nT)',fontsize=24)
        ax1.set_xlabel('Time Series',fontsize=24)
        ax1.tick_params(axis='x', labelsize=18)  # Change to desired size
        ax1.tick_params(axis='y', labelsize=18)
        
        ax1.plot(mag_datetime_arr,Br,color='tab:blue',label="Non-Quiescent Br",zorder=1)
        ax1.plot(mag_datetime_arr,B,color='black',label="|B| Magnitude",zorder=3)
        ax1.plot(q_mag_datetime,q_mag_Br,color='tab:orange',label="Delicious Quiescent Br",zorder=2)
        ...
        ax2.set_ylabel('Hamogram 2m',fontsize=24)
        ax2.set_xlabel('Time Series',fontsize=24)
        ax2.tick_params(axis='x', labelsize=18)  # Change to desired size
        ax2.tick_params(axis='y', labelsize=18)
        
        # Change the text size of the offset label
        offset_text = ax2.xaxis.get_offset_text()  # Get the offset label
        offset_text.set_size(20)  # Set font size (e.g., 14, 16)
        
        # axs.plot(ham_datetime_arr,ham_data_arr,color='tab:blue')
        # axs.plot(q_ham_datetime,q_ham,color='tab:orange')
        
        ax2.plot(ham_datetime_arr,ham_data_arr,color='tab:blue',label="Non-Quiescent Ham")
        ax2.plot(q_ham_datetime,q_ham,color='tab:orange',label="Delicious Quiescent Ham")
        plt.legend(fontsize=20)
        fig.subplots_adjust(hspace=0)  # Zero spacing between subplots
        plt.show()
